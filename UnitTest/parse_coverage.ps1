...
foreach ($r in $rows) {
    ...
    if ($uncov -gt 15 -and $pct -lt 70 -and $total -lt 500 -and $total -gt 30) {
        $results += [PSCustomObject]@{Unit=$name; Covered=$covered; Uncovered=$uncov; Total=$total; Coverage="$pct%"}
    }
}
$results | Sort-Object -Property Uncovered -Descending | Select-Object -First 25 | Format-Table -AutoSize
