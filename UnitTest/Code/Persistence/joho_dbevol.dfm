object dmTestCase: TdmTestCase
  OldCreateOrder = False
  Height = 594
  Width = 740
  object BoldSystemHandle1: TBoldSystemHandle
    IsDefault = True
    SystemTypeInfoHandle = BoldSystemTypeInfoHandle1
    Active = False
    PersistenceHandle = BoldPersistenceHandleDB1
    Left = 168
    Top = 260
  end
  object BoldSystemTypeInfoHandle1: TBoldSystemTypeInfoHandle
    BoldModel = BoldModel1
    UseGeneratedCode = False
    Left = 168
    Top = 212
  end
  object BoldModel1: TBoldModel
    UMLModelMode = ummNone
    Boldify.EnforceDefaultUMLCase = False
    Boldify.DefaultNavigableMultiplicity = '0..1'
    Boldify.DefaultNonNavigableMultiplicity = '0..*'
    Left = 32
    Top = 24
    Model = (
      'VERSION 19'
      '(Model'
      #9'"BusinessClasses"'
      #9'"Root"'
      #9'""'
      #9'""'
      
        #9'"_BoldInternal.flattened=True,_Boldify.boldified=True,_BoldInte' +
        'rnal.ModelErrors=,Bold.DelphiName=<Name>,Bold.RootClass=Root"'
      #9'(Classes'
      #9#9'(Class'
      #9#9#9'"Root"'
      #9#9#9'"<NONE>"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"RootAttr"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.AllowNULL=True"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"ClassA"'
      #9#9#9'"Root"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"Attr1"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.AllowNULL=True"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"ClassB"'
      #9#9#9'"Root"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"Attr2"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.AllowNULL=True"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"MidClass"'
      #9#9#9'"Root"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"MidAttr"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.AllowNULL=True"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"SubClass"'
      #9#9#9'"MidClass"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"SubAttr1"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.AllowNULL=True"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"SubClass2"'
      #9#9#9'"MidClass"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"SubAttr2"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.AllowNULL=True"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"SubSub"'
      #9#9#9'"SubClass2"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9')'
      #9'(Associations'
      #9')'
      ')')
  end
  object BoldUMLRose98Link1: TBoldUMLRoseLink
    FileName = 'D:\bold\BfD\TestCases\Persistence\DbEvol1.mdl'
    BoldModel = BoldModel1
    Left = 116
    Top = 24
  end
  object BoldModel2: TBoldModel
    UMLModelMode = ummNone
    Boldify.EnforceDefaultUMLCase = False
    Boldify.DefaultNavigableMultiplicity = '0..1'
    Boldify.DefaultNonNavigableMultiplicity = '0..*'
    Left = 32
    Top = 80
    Model = (
      'VERSION 19'
      '(Model'
      #9'"BusinessClasses"'
      #9'"Root"'
      #9'""'
      #9'""'
      
        #9'"_BoldInternal.flattened=True,_Boldify.boldified=True,_BoldInte' +
        'rnal.ModelErrors=,Bold.DelphiName=<Name>,Bold.RootClass=Root"'
      #9'(Classes'
      #9#9'(Class'
      #9#9#9'"Root"'
      #9#9#9'"<NONE>"'
      #9#9#9'TRUE'
      #9#9#9'TRUE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"RootAttr"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.AllowNULL=True"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"ClassA2"'
      #9#9#9'"Root"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent,Bold.FormerNames=ClassA"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"Attr1"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.AllowNULL=True"'
      #9#9#9#9')'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"NewAttr"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.AllowNULL=True,Bold.DefaultDBValue='#39#39'"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"ClassB"'
      #9#9#9'"MidClass"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"Attr2"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.AllowNULL=True"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"MidClass"'
      #9#9#9'"Root"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"MidAttr"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.AllowNULL=True"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"SubClass"'
      #9#9#9'"MidClass"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent,Bold.TableMapping=Parent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"SubAttr1"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.AllowNULL=True"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"SubClass2"'
      #9#9#9'"Root"'
      #9#9#9'TRUE'
      #9#9#9'TRUE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent,Bold.TableMapping=Children"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"SubAttr2"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.AllowNULL=True"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"SubSub"'
      #9#9#9'"SubClass2"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"NewClass"'
      #9#9#9'"Root"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent,Bold.FormerNames=AnotherNewClass"'
      #9#9#9'(Attributes'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9')'
      #9'(Associations'
      #9#9'(Association'
      #9#9#9'"SubClassClassB"'
      #9#9#9'"<NONE>"'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transien' +
        't\",derived=False,Bold.DelphiName=<Name>"'
      #9#9#9'FALSE'
      #9#9#9'(Roles'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"SubClass"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"ClassB"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"*"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      #9#9#9#9#9'"Bold.Embed=False"'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"ClassB"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"SubClass"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"0..1"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      #9#9#9#9#9'""'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9')'
      #9#9')'
      #9')'
      ')')
  end
  object BoldUMLRose98Link2: TBoldUMLRoseLink
    FileName = 'D:\bold\BfD\TestCases\Persistence\dbEvol2.mdl'
    BoldModel = BoldModel2
    Left = 120
    Top = 80
  end
  object bmoMain: TBoldModel
    UMLModelMode = ummNone
    Boldify.EnforceDefaultUMLCase = False
    Boldify.DefaultNavigableMultiplicity = '0..1'
    Boldify.DefaultNonNavigableMultiplicity = '0..*'
    Left = 376
    Top = 24
    Model = (
      'VERSION 19'
      '(Model'
      #9'"ModelEv"'
      #9'"BusinessClassesRoot"'
      #9'""'
      #9'""'
      
        #9'"_BoldInternal.flattened=True,_Boldify.boldified=True,_BoldInte' +
        'rnal.ModelErrors=,Bold.DelphiName=<Name>,Bold.UnitName=ModelEv,B' +
        'old.RootClass=BusinessClassesRoot"'
      #9'(Classes'
      #9#9'(Class'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'"<NONE>"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.TableName=<Prefix>_OBJECT,Bold.V' +
        'ersioned=<Default>"'
      #9#9#9'(Attributes'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"A"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"a1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"a2"'
      #9#9#9#9#9'"integer"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"B"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"b1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"b2"'
      #9#9#9#9#9'"integer"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"C"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"C1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"SubB"'
      #9#9#9'"B"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"SubB1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"CSub"'
      #9#9#9'"C"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"CSub1"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"CSubSub1"'
      #9#9#9'"CSub"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"CSubSub2"'
      #9#9#9'"CSub"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"CSubSub1"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"BaRole"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent,Bold.Versioned=<Default>"'
      #9#9#9'(Attributes'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9')'
      #9'(Associations'
      #9#9'(Association'
      #9#9#9'"BaRole"'
      #9#9#9'"BaRole"'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transien' +
        't\",derived=False,Bold.DelphiName=<Name>"'
      #9#9#9'FALSE'
      #9#9#9'(Roles'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"B"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"A"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"*"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      #9#9#9#9#9'"Bold.Embed=False"'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"aRole"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"B"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"*"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      #9#9#9#9#9'"Bold.Embed=False"'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9')'
      #9#9')'
      #9#9'(Association'
      #9#9#9'"BC"'
      #9#9#9'"<NONE>"'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transien' +
        't\",derived=False,Bold.DelphiName=<Name>"'
      #9#9#9'FALSE'
      #9#9#9'(Roles'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"B"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"C"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"0..1"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      #9#9#9#9#9'""'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"C"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"B"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"*"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      #9#9#9#9#9'"Bold.Embed=False"'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9')'
      #9#9')'
      #9')'
      ')')
  end
  object bmoAdd: TBoldModel
    UMLModelMode = ummNone
    Boldify.EnforceDefaultUMLCase = False
    Boldify.DefaultNavigableMultiplicity = '0..1'
    Boldify.DefaultNonNavigableMultiplicity = '0..*'
    Left = 376
    Top = 80
    Model = (
      'VERSION 19'
      '(Model'
      #9'"ModelEv"'
      #9'"BusinessClassesRoot"'
      #9'""'
      #9'""'
      
        #9'"_BoldInternal.flattened=True,_Boldify.boldified=True,_BoldInte' +
        'rnal.ModelErrors=,Bold.DelphiName=<Name>,Bold.UnitName=ModelEv,B' +
        'old.RootClass=BusinessClassesRoot"'
      #9'(Classes'
      #9#9'(Class'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'"<NONE>"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.TableName=<Prefix>_OBJECT,Bold.V' +
        'ersioned=<Default>"'
      #9#9#9'(Attributes'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"A"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"a1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"a2"'
      #9#9#9#9#9'"integer"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"NewA3"'
      #9#9#9#9#9'"Integer"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.DefaultDBValue=1"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"B"'
      #9#9#9'"NewSuper"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"b1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"b2"'
      #9#9#9#9#9'"integer"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"NewSuper"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"NewSuper1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.DefaultDBValue='#39#39'"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"C"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"C1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"SubB"'
      #9#9#9'"B"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"SubB1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"CSub"'
      #9#9#9'"C"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"CSub1"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"CSubSub1"'
      #9#9#9'"CSub"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"CSubSub2"'
      #9#9#9'"CSub"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"CSubSub1"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"NewClass"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"New1"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"NewSubClass"'
      #9#9#9'"A"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"NewSub1"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"NewLinkClass"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent,\"Bold.FormerNames=aRoleB, BaRole\""'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"LinkAttr1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.DefaultDBValue=Hej"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9')'
      #9'(Associations'
      #9#9'(Association'
      #9#9#9'"BaRole"'
      #9#9#9'"NewLinkClass"'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transien' +
        't\",derived=False,Bold.DelphiName=<Name>"'
      #9#9#9'FALSE'
      #9#9#9'(Roles'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"B"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"A"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"*"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      #9#9#9#9#9'"Bold.Embed=False"'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"aRole"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"B"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"*"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      #9#9#9#9#9'"Bold.Embed=False"'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9')'
      #9#9')'
      #9#9'(Association'
      #9#9#9'"BC"'
      #9#9#9'"<NONE>"'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transien' +
        't\",derived=False,Bold.DelphiName=<Name>"'
      #9#9#9'FALSE'
      #9#9#9'(Roles'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"B"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"C"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"0..1"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      #9#9#9#9#9'""'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"C"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"B"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"*"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      #9#9#9#9#9'"Bold.Embed=False"'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9')'
      #9#9')'
      #9#9'(Association'
      #9#9#9'"AaNewRoleb"'
      #9#9#9'"<NONE>"'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transien' +
        't\",derived=False,Bold.DelphiName=<Name>"'
      #9#9#9'FALSE'
      #9#9#9'(Roles'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"A"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"B"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"*"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      #9#9#9#9#9'"Bold.Embed=False"'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"aNewRoleb"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"A"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"1"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      #9#9#9#9#9'""'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9')'
      #9#9')'
      #9')'
      ')')
  end
  object brlMain: TBoldUMLRoseLink
    FileName = 'D:\bold\BfD\TestCases\Persistence\ModelEv_Base.mdl'
    BoldModel = bmoMain
    Left = 456
    Top = 24
  end
  object brlAdd: TBoldUMLRoseLink
    FileName = 'D:\bold\BfD\TestCases\Persistence\ModelEv_Add.mdl'
    BoldModel = bmoAdd
    Left = 456
    Top = 80
  end
  object bmoRemove: TBoldModel
    UMLModelMode = ummNone
    Boldify.EnforceDefaultUMLCase = False
    Boldify.DefaultNavigableMultiplicity = '0..1'
    Boldify.DefaultNonNavigableMultiplicity = '0..*'
    Left = 384
    Top = 144
    Model = (
      'VERSION 19'
      '(Model'
      #9'"ModelEv"'
      #9'"BusinessClassesRoot"'
      #9'""'
      #9'""'
      
        #9'"Bold.DelphiName=<Name>,Bold.Constraints=,Bold.DefaultMemberInf' +
        'oClass=,Bold.DefaultMemberInfoClassName=,Bold.ExpressionName=<Na' +
        'me>,Bold.MaxDBIdentifierLength=-1,Bold.ModelName=ModelEv,\"Bold.' +
        'NationalCharConversionEnum=<Default>, True, False\",\"Bold.Optim' +
        'isticLockingSet=<Default>, Off, Member, Class, TimeStamp\",Bold.' +
        'PTYVersion=1,Bold.PTYVersionSet=1,Bold.UnitName=ModelEv"'
      #9'(Classes'
      #9#9'(Class'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'"<NONE>"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.TableName=<Prefix>_OBJECT,Bold.V' +
        'ersioned=<Default>"'
      #9#9#9'(Attributes'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"A"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\""'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"a1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClass=<Default>,Bold.MemberInfoClassName=<D' +
        'efault>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLockin' +
        'gSet=<Default>, Off, Member, Class, TimeStamp\",Bold.RemovalStat' +
        'e=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"B"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\""'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"b1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClass=<Default>,Bold.MemberInfoClassName=<D' +
        'efault>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLockin' +
        'gSet=<Default>, Off, Member, Class, TimeStamp\",Bold.RemovalStat' +
        'e=(200)"'
      #9#9#9#9')'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"b2"'
      #9#9#9#9#9'"integer"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClass=<Default>,Bold.MemberInfoClassName=<D' +
        'efault>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLockin' +
        'gSet=<Default>, Off, Member, Class, TimeStamp\",Bold.RemovalStat' +
        'e=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"C"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\""'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"C1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClass=<Default>,Bold.MemberInfoClassName=<D' +
        'efault>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLockin' +
        'gSet=<Default>, Off, Member, Class, TimeStamp\",Bold.RemovalStat' +
        'e=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"CSubSub1"'
      #9#9#9'"C"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\""'
      #9#9#9'(Attributes'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"CSubSub2"'
      #9#9#9'"C"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\""'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"CSubSub1"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClass=<Default>,Bold.MemberInfoClassName=<D' +
        'efault>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLockin' +
        'gSet=<Default>, Off, Member, Class, TimeStamp\",Bold.RemovalStat' +
        'e=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"BaRole"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent,Bold.Versioned=<Default>"'
      #9#9#9'(Attributes'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9')'
      #9'(Associations'
      #9#9'(Association'
      #9#9#9'"BaRole"'
      #9#9#9'"BaRole"'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transien' +
        't\",Bold.DelphiName=<Name>,\"Bold.EvolutionStateEnum=Normal, ToB' +
        'eRemoved, Removed\",Bold.ExpressionName=<Name>,Bold.LinkClassId=' +
        '-1,Bold.PMapper=<Default>,Bold.TableMapping=Own"'
      #9#9#9'FALSE'
      #9#9#9'(Roles'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"B"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"A"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"*"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      
        #9#9#9#9#9'"Bold.Changeability=changeable,\"Bold.ChangeabilityKind=add' +
        'Only, changeable, frozen\",Bold.Constraints=,\"Bold.DeleteAction' +
        's=<Default>, Allow, Prohibit, Cascade\",Bold.Embed=False,Bold.Me' +
        'mberInfoClass=<Default>,Bold.MemberInfoClassName=<Default>,Bold.' +
        'OptimisticLocking=<Default>,\"Bold.OptimisticLockingSet=<Default' +
        '>, Off, Member, Class, TimeStamp\",Bold.PMapper=<Default>"'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"aRole"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"B"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"*"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      
        #9#9#9#9#9'"Bold.Changeability=changeable,\"Bold.ChangeabilityKind=add' +
        'Only, changeable, frozen\",Bold.Constraints=,\"Bold.DeleteAction' +
        's=<Default>, Allow, Prohibit, Cascade\",Bold.Embed=False,Bold.Me' +
        'mberInfoClass=<Default>,Bold.MemberInfoClassName=<Default>,Bold.' +
        'OptimisticLocking=<Default>,\"Bold.OptimisticLockingSet=<Default' +
        '>, Off, Member, Class, TimeStamp\",Bold.PMapper=<Default>"'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9')'
      #9#9')'
      #9')'
      ')')
  end
  object brlRemove: TBoldUMLRoseLink
    FileName = 'D:\bold\BfD\TestCases\Persistence\ModelEv_Remove.mdl'
    BoldModel = bmoRemove
    Left = 456
    Top = 144
  end
  object bmoChange: TBoldModel
    UMLModelMode = ummNone
    Boldify.EnforceDefaultUMLCase = False
    Boldify.DefaultNavigableMultiplicity = '0..1'
    Boldify.DefaultNonNavigableMultiplicity = '0..*'
    Left = 384
    Top = 208
    Model = (
      'VERSION 19'
      '(Model'
      #9'"ModelEv"'
      #9'"BusinessClassesRoot"'
      #9'""'
      #9'""'
      
        #9'"_BoldInternal.flattened=True,_Boldify.boldified=True,_BoldInte' +
        'rnal.ModelErrors=,Bold.DelphiName=<Name>,Bold.UnitName=ModelEv,B' +
        'old.RootClass=BusinessClassesRoot"'
      #9'(Classes'
      #9#9'(Class'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'"<NONE>"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.TableName=<Prefix>_OBJECT,Bold.V' +
        'ersioned=<Default>"'
      #9#9#9'(Attributes'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"NewA"'
      #9#9#9'"B"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent,Bold.FormerNames=A"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"a1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"a2"'
      #9#9#9#9#9'"integer"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.DelayedFetch=True"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"B"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"NewB1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.FormerNames=b1"'
      #9#9#9#9')'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"SubB1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"C"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'TRUE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent,Bold.TableMapping=Children"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"C1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"SubB"'
      #9#9#9'"NewSubB"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"b2"'
      #9#9#9#9#9'"integer"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"NewSubB"'
      #9#9#9'"B"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"NewSubB1"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False,Bold.DefaultDBValue=h'#229'"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"CSub"'
      #9#9#9'"C"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"CSub1"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"CSubSub1"'
      #9#9#9'"C"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent"'
      #9#9#9'(Attributes'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"CSubSub2"'
      #9#9#9'"CSub"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent,Bold.TableMapping=Parent"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"CSubSub1"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",derived=False"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"BanotherRole"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      #9#9#9'"persistence=Persistent,Bold.Versioned=<Default>"'
      #9#9#9'(Attributes'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9')'
      #9'(Associations'
      #9#9'(Association'
      #9#9#9'"BanotherRole"'
      #9#9#9'"BanotherRole"'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transien' +
        't\",derived=False,Bold.DelphiName=<Name>"'
      #9#9#9'FALSE'
      #9#9#9'(Roles'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"B"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"NewA"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"*"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      #9#9#9#9#9'"Bold.Embed=False"'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"anotherRole"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"B"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"*"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      #9#9#9#9#9'"Bold.Embed=False,Bold.FormerNames=aRole"'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9')'
      #9#9')'
      #9#9'(Association'
      #9#9#9'"BC"'
      #9#9#9'"<NONE>"'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transien' +
        't\",derived=False,Bold.DelphiName=<Name>"'
      #9#9#9'FALSE'
      #9#9#9'(Roles'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"B"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"C"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"0..1"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      #9#9#9#9#9'""'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"C"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"B"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"*"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      #9#9#9#9#9'"Bold.Embed=False"'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9')'
      #9#9')'
      #9')'
      ')')
  end
  object brlChange: TBoldUMLRoseLink
    FileName = 'D:\bold\BfD\TestCases\Persistence\ModelEv_Change.mdl'
    BoldModel = bmoChange
    Left = 456
    Top = 208
  end
  object bmotempBase: TBoldModel
    UMLModelMode = ummNone
    Boldify.EnforceDefaultUMLCase = False
    Boldify.DefaultNavigableMultiplicity = '0..1'
    Boldify.DefaultNonNavigableMultiplicity = '0..*'
    Left = 360
    Top = 344
    Model = (
      'VERSION 19'
      '(Model'
      #9'"ModelEvTemporal"'
      #9'"TempRootClass"'
      #9'""'
      #9'""'
      
        #9'"Bold.DelphiName=<Name>,Bold.Constraints=,Bold.DefaultMemberInf' +
        'oClass=,Bold.DefaultMemberInfoClassName=,Bold.ExpressionName=<Na' +
        'me>,Bold.MaxDBIdentifierLength=-1,Bold.ModelName=ModelEvTemporal' +
        ',\"Bold.NationalCharConversionEnum=<Default>, True, False\",\"Bo' +
        'ld.OptimisticLockingSet=<Default>, Off, Member, Class, TimeStamp' +
        '\",Bold.PTYVersion=1,Bold.PTYVersionSet=1,Bold.RootClass=TempRoo' +
        'tClass,Bold.UnitName=ModelEvTemporal"'
      #9'(Classes'
      #9#9'(Class'
      #9#9#9'"TempRootClass"'
      #9#9#9'"<NONE>"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\",Bold.Versioned=True"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"RootAttr"'
      #9#9#9#9#9'"integer"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClass=<Default>,Bold.MemberInfoClassName=<D' +
        'efault>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLockin' +
        'gSet=<Default>, Off, Member, Class, TimeStamp\",Bold.RemovalStat' +
        'e=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"TempA"'
      #9#9#9'"TempRootClass"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\",Bold.Versioned=True"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"A1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClass=<Default>,Bold.MemberInfoClassName=<D' +
        'efault>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLockin' +
        'gSet=<Default>, Off, Member, Class, TimeStamp\",Bold.RemovalStat' +
        'e=(200)"'
      #9#9#9#9')'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"a2"'
      #9#9#9#9#9'"integer"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClass=<Default>,Bold.MemberInfoClassName=<D' +
        'efault>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLockin' +
        'gSet=<Default>, Off, Member, Class, TimeStamp\",Bold.RemovalStat' +
        'e=(200)"'
      #9#9#9#9')'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"a3"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClass=<Default>,Bold.MemberInfoClassName=<D' +
        'efault>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLockin' +
        'gSet=<Default>, Off, Member, Class, TimeStamp\",Bold.RemovalStat' +
        'e=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"TempB"'
      #9#9#9'"TempA"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\",Bold.Versioned=True"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"b1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClass=<Default>,Bold.MemberInfoClassName=<D' +
        'efault>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLockin' +
        'gSet=<Default>, Off, Member, Class, TimeStamp\",Bold.RemovalStat' +
        'e=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"TempUnversioned"'
      #9#9#9'"TempA"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\""'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"u1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClass=<Default>,Bold.MemberInfoClassName=<D' +
        'efault>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLockin' +
        'gSet=<Default>, Off, Member, Class, TimeStamp\",Bold.RemovalStat' +
        'e=(200)"'
      #9#9#9#9')'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"u2"'
      #9#9#9#9#9'"integer"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClass=<Default>,Bold.MemberInfoClassName=<D' +
        'efault>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLockin' +
        'gSet=<Default>, Off, Member, Class, TimeStamp\",Bold.RemovalStat' +
        'e=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9')'
      #9'(Associations'
      #9')'
      ')')
  end
  object bmotempChange: TBoldModel
    UMLModelMode = ummNone
    Boldify.EnforceDefaultUMLCase = False
    Boldify.DefaultNavigableMultiplicity = '0..1'
    Boldify.DefaultNonNavigableMultiplicity = '0..*'
    Left = 360
    Top = 408
    Model = (
      'VERSION 19'
      '(Model'
      #9'"ModelEvTemporal"'
      #9'"TempRootClass"'
      #9'""'
      #9'""'
      
        #9'"Bold.DelphiName=<Name>,Bold.Constraints=,Bold.DefaultMemberInf' +
        'oClass=,Bold.DefaultMemberInfoClassName=,Bold.ExpressionName=<Na' +
        'me>,Bold.MaxDBIdentifierLength=-1,Bold.ModelName=ModelEvTemporal' +
        ',\"Bold.NationalCharConversionEnum=<Default>, True, False\",\"Bo' +
        'ld.OptimisticLockingSet=<Default>, Off, Member, Class, TimeStamp' +
        '\",Bold.PTYVersion=1,Bold.PTYVersionSet=1,Bold.RootClass=TempRoo' +
        'tClass,Bold.UnitName=ModelEvTemporal"'
      #9'(Classes'
      #9#9'(Class'
      #9#9#9'"TempRootClass"'
      #9#9#9'"<NONE>"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\",Bold.Versioned=True"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"RootAttr"'
      #9#9#9#9#9'"integer"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClass=<Default>,Bold.MemberInfoClassName=<D' +
        'efault>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLockin' +
        'gSet=<Default>, Off, Member, Class, TimeStamp\",Bold.RemovalStat' +
        'e=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"TempA"'
      #9#9#9'"TempRootClass"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\",Bold.Versioned=True"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"u2"'
      #9#9#9#9#9'"integer"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClass=<Default>,Bold.MemberInfoClassName=<D' +
        'efault>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLockin' +
        'gSet=<Default>, Off, Member, Class, TimeStamp\",Bold.RemovalStat' +
        'e=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"TempB"'
      #9#9#9'"TempBSuper"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\",Bold.Versioned=True"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"b1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClass=<Default>,Bold.MemberInfoClassName=<D' +
        'efault>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLockin' +
        'gSet=<Default>, Off, Member, Class, TimeStamp\",Bold.RemovalStat' +
        'e=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"TempBSuper"'
      #9#9#9'"TempA"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\",Bold.Versioned=True"'
      #9#9#9'(Attributes'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"TempUnversioned"'
      #9#9#9'"TempUSuper"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\""'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"a2"'
      #9#9#9#9#9'"integer"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClass=<Default>,Bold.MemberInfoClassName=<D' +
        'efault>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLockin' +
        'gSet=<Default>, Off, Member, Class, TimeStamp\",Bold.RemovalStat' +
        'e=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"TempUSuper"'
      #9#9#9'"TempASub"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\""'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"a3"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.ColumnName=New' +
        '<Name>,Bold.Constraints=,\"Bold.DelphiPropertySet=None, Field, P' +
        'rivateMethod, ProtectedVirtualMethod\",\"Bold.EvolutionStateEnum' +
        '=Normal, ToBeRemoved, Removed\",Bold.MemberInfoClass=<Default>,B' +
        'old.MemberInfoClassName=<Default>,Bold.OptimisticLocking=<Defaul' +
        't>,\"Bold.OptimisticLockingSet=<Default>, Off, Member, Class, Ti' +
        'meStamp\",Bold.RemovalState=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"TempASub"'
      #9#9#9'"TempA"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\",Bold.Versioned=True"'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"a1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClass=<Default>,Bold.MemberInfoClassName=<D' +
        'efault>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLockin' +
        'gSet=<Default>, Off, Member, Class, TimeStamp\",Bold.RemovalStat' +
        'e=(200)"'
      #9#9#9#9')'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"u1"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClass=<Default>,Bold.MemberInfoClassName=<D' +
        'efault>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLockin' +
        'gSet=<Default>, Off, Member, Class, TimeStamp\",Bold.RemovalStat' +
        'e=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9')'
      #9'(Associations'
      #9')'
      ')')
  end
  object brlTempBase: TBoldUMLRoseLink
    FileName = 'D:\bold\BfD\TestCases\Persistence\ModelEvTemporal_Base.mdl'
    BoldModel = bmotempBase
    Left = 464
    Top = 344
  end
  object brlTempChange: TBoldUMLRoseLink
    FileName = 'D:\bold\BfD\TestCases\Persistence\ModelEvTemporal_Change.mdl'
    BoldModel = bmotempChange
    Left = 464
    Top = 408
  end
  object bmoUpgrader1: TBoldModel
    UMLModelMode = ummNone
    Boldify.EnforceDefaultUMLCase = False
    Boldify.DefaultNavigableMultiplicity = '0..1'
    Boldify.DefaultNonNavigableMultiplicity = '0..*'
    Left = 32
    Top = 384
    Model = (
      'VERSION 19'
      '(Model'
      #9'"ModEv_Upgrade"'
      #9'"BusinessClassesRoot"'
      #9'""'
      #9'""'
      
        #9'"Bold.DelphiName=<Name>,Bold.Constraints=,Bold.DefaultMemberInf' +
        'oClassName=,Bold.ExpressionName=<Name>,Bold.MaxDBIdentifierLengt' +
        'h=-1,Bold.ModelName=ModEv_Upgrade,Bold.ModelVersion=1,\"Bold.Nat' +
        'ionalCharConversionEnum=<Default>, True, False\",\"Bold.Optimist' +
        'icLockingSet=<Default>, Off, Member, Class, TimeStamp\",Bold.PTY' +
        'Version=1,Bold.PTYVersionSet=1,Bold.UnitName=ModEv_Upgrade,Bold.' +
        'UseModelVersion=True"'
      #9'(Classes'
      #9#9'(Class'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'"<NONE>"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.TableName=<Prefix>_OBJECT,Bold.V' +
        'ersioned=<Default>"'
      #9#9#9'(Attributes'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"UpA"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\""'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"Name"'
      #9#9#9#9#9'"integer"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClassName=<Default>,Bold.OptimisticLocking=' +
        '<Default>,\"Bold.OptimisticLockingSet=<Default>, Off, Member, Cl' +
        'ass, TimeStamp\",Bold.RemovalState=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"UpB"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\""'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"Amount"'
      #9#9#9#9#9'"Currency"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClassName=<Default>,Bold.OptimisticLocking=' +
        '<Default>,\"Bold.OptimisticLockingSet=<Default>, Off, Member, Cl' +
        'ass, TimeStamp\",Bold.RemovalState=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"UpC"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\""'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"name"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClassName=<Default>,Bold.OptimisticLocking=' +
        '<Default>,\"Bold.OptimisticLockingSet=<Default>, Off, Member, Cl' +
        'ass, TimeStamp\",Bold.RemovalState=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"UpD"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\""'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"anAttr"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClassName=<Default>,Bold.OptimisticLocking=' +
        '<Default>,\"Bold.OptimisticLockingSet=<Default>, Off, Member, Cl' +
        'ass, TimeStamp\",Bold.RemovalState=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9')'
      #9'(Associations'
      #9')'
      ')')
  end
  object bmoUpgrader2: TBoldModel
    UMLModelMode = ummNone
    Boldify.EnforceDefaultUMLCase = False
    Boldify.DefaultNavigableMultiplicity = '0..1'
    Boldify.DefaultNonNavigableMultiplicity = '0..*'
    Left = 32
    Top = 432
    Model = (
      'VERSION 19'
      '(Model'
      #9'"ModEv_Upgrade"'
      #9'"BusinessClassesRoot"'
      #9'""'
      #9'""'
      
        #9'"Bold.DelphiName=<Name>,Bold.Constraints=,Bold.DefaultMemberInf' +
        'oClassName=,Bold.ExpressionName=<Name>,Bold.MaxDBIdentifierLengt' +
        'h=-1,Bold.ModelName=ModEv_Upgrade,Bold.ModelVersion=2,\"Bold.Nat' +
        'ionalCharConversionEnum=<Default>, True, False\",\"Bold.Optimist' +
        'icLockingSet=<Default>, Off, Member, Class, TimeStamp\",Bold.PTY' +
        'Version=1,Bold.PTYVersionSet=1,Bold.UnitName=ModEv_Upgrade,Bold.' +
        'UseModelVersion=True"'
      #9'(Classes'
      #9#9'(Class'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'"<NONE>"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.TableName=<Prefix>_OBJECT,Bold.V' +
        'ersioned=<Default>"'
      #9#9#9'(Attributes'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"UpA"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\""'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"Name"'
      #9#9#9#9#9'"integer"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClassName=<Default>,Bold.OptimisticLocking=' +
        '<Default>,\"Bold.OptimisticLockingSet=<Default>, Off, Member, Cl' +
        'ass, TimeStamp\",Bold.RemovalState=(200)"'
      #9#9#9#9')'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"NewName"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClassName=<Default>,Bold.OptimisticLocking=' +
        '<Default>,\"Bold.OptimisticLockingSet=<Default>, Off, Member, Cl' +
        'ass, TimeStamp\",Bold.RemovalState=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"UpB"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\""'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"Amount"'
      #9#9#9#9#9'"Currency"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClassName=<Default>,Bold.OptimisticLocking=' +
        '<Default>,\"Bold.OptimisticLockingSet=<Default>, Off, Member, Cl' +
        'ass, TimeStamp\",Bold.RemovalState=(200)"'
      #9#9#9#9')'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"EuroAmount"'
      #9#9#9#9#9'"currency"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClassName=<Default>,Bold.OptimisticLocking=' +
        '<Default>,\"Bold.OptimisticLockingSet=<Default>, Off, Member, Cl' +
        'ass, TimeStamp\",Bold.RemovalState=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"UpC"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\""'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"name"'
      #9#9#9#9#9'"string"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClassName=<Default>,Bold.OptimisticLocking=' +
        '<Default>,\"Bold.OptimisticLockingSet=<Default>, Off, Member, Cl' +
        'ass, TimeStamp\",Bold.RemovalState=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9#9'(Class'
      #9#9#9'"UpD"'
      #9#9#9'"BusinessClassesRoot"'
      #9#9#9'TRUE'
      #9#9#9'FALSE'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,Bold.Constraints=,\"Bold.EvolutionSta' +
        'teEnum=Normal, ToBeRemoved, Removed\",\"Bold.OptimisticLockingSe' +
        't=<Default>, Off, Member, Class, TimeStamp\",\"Bold.TableMapping' +
        'Set=Own, Parent, Children, Imported\""'
      #9#9#9'(Attributes'
      #9#9#9#9'(Attribute'
      #9#9#9#9#9'"anAttr"'
      #9#9#9#9#9'"String"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'""'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'""'
      
        #9#9#9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transi' +
        'ent\",\"Bold.AttributeKindSet=Bold, Delphi\",Bold.Constraints=,\' +
        '"Bold.DelphiPropertySet=None, Field, PrivateMethod, ProtectedVir' +
        'tualMethod\",\"Bold.EvolutionStateEnum=Normal, ToBeRemoved, Remo' +
        'ved\",Bold.MemberInfoClassName=<Default>,Bold.OptimisticLocking=' +
        '<Default>,\"Bold.OptimisticLockingSet=<Default>, Off, Member, Cl' +
        'ass, TimeStamp\",Bold.RemovalState=(200)"'
      #9#9#9#9')'
      #9#9#9')'
      #9#9#9'(Methods'
      #9#9#9')'
      #9#9')'
      #9')'
      #9'(Associations'
      #9#9'(Association'
      #9#9#9'"UpDx__UpC"'
      #9#9#9'"<NONE>"'
      #9#9#9'""'
      #9#9#9'""'
      
        #9#9#9'"persistence=Persistent,\"PersistenceSet=Persistent, Transien' +
        't\",Bold.DelphiName=<Name>,\"Bold.EvolutionStateEnum=Normal, ToB' +
        'eRemoved, Removed\",Bold.ExpressionName=<Name>,Bold.PMapper=<Def' +
        'ault>,Bold.TableMapping=Own"'
      #9#9#9'FALSE'
      #9#9#9'(Roles'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"UpD"'
      #9#9#9#9#9'TRUE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"UpC"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"1"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'2'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      
        #9#9#9#9#9'"Bold.Changeability=changeable,\"Bold.ChangeabilityKind=add' +
        'Only, changeable, frozen\",Bold.Constraints=,\"Bold.DeleteAction' +
        's=<Default>, Allow, Prohibit, Cascade\",Bold.MemberInfoClassName' +
        '=<Default>,Bold.OptimisticLocking=<Default>,\"Bold.OptimisticLoc' +
        'kingSet=<Default>, Off, Member, Class, TimeStamp\",Bold.PMapper=' +
        '<Default>"'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9#9'(Role'
      #9#9#9#9#9'"x__UpC"'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'FALSE'
      #9#9#9#9#9'"UpD"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'"1"'
      #9#9#9#9#9'""'
      #9#9#9#9#9'0'
      #9#9#9#9#9'2'
      #9#9#9#9#9'0'
      
        #9#9#9#9#9'"Bold.Changeability=changeable,\"Bold.ChangeabilityKind=add' +
        'Only, changeable, frozen\",Bold.Constraints=,\"Bold.DeleteAction' +
        's=<Default>, Allow, Prohibit, Cascade\",Bold.Embed=False,Bold.Me' +
        'mberInfoClassName=<Default>,Bold.OptimisticLocking=<Default>,\"B' +
        'old.OptimisticLockingSet=<Default>, Off, Member, Class, TimeStam' +
        'p\",Bold.PMapper=<Default>"'
      #9#9#9#9#9'(Qualifiers'
      #9#9#9#9#9')'
      #9#9#9#9')'
      #9#9#9')'
      #9#9')'
      #9')'
      ')')
  end
  object brlUpgrader1: TBoldUMLRoseLink
    FileName = 'D:\Bold\BfD\TestCases\Persistence\ModelEv_ObjectUpgrade_v1.mdl'
    BoldModel = bmoUpgrader1
    Left = 120
    Top = 384
  end
  object brlUpgrader2: TBoldUMLRoseLink
    FileName = 'D:\Bold\BfD\TestCases\Persistence\ModelEv_ObjectUpgrade_v2.mdl'
    BoldModel = bmoUpgrader2
    Left = 120
    Top = 432
  end
  object BoldObjectUpgraderHandle1: TBoldObjectUpgraderHandle
    Config = <
      item
        ExpressionName = 'UpA'
        UpgradeOlderThanVersion = 2
        OnUpgradeObject = BoldObjectUpgraderHandle1_UpA_UpgradeObject
      end
      item
        ExpressionName = 'UpB'
        UpgradeOlderThanVersion = 2
        OnUpgradeObject = BoldObjectUpgraderHandle1_UpB_UpgradeObject
      end
      item
        ExpressionName = 'UpC'
        UpgradeOlderThanVersion = 2
      end
      item
        ExpressionName = 'jeppe___'
        UpgradeOlderThanVersion = 0
      end>
    PersistenceHandle = BoldPersistenceHandleDB1
    SystemTypeInfoHandle = BoldSystemTypeInfoHandle1
    OnUpgradeObject = BoldObjectUpgraderHandle1UpgradeObject
    Left = 56
    Top = 296
  end
  object BoldPersistenceHandleDB1: TBoldPersistenceHandleDB
    BoldModel = BoldModel1
    ClockLogGranularity = '0:0:0.0'
    DatabaseAdapter = BoldDatabaseAdapterUniDAC1
    Left = 280
    Top = 192
  end
  object BoldDatabaseAdapterUniDAC1: TBoldDatabaseAdapterUniDAC
    SQLDatabaseConfig.DefaultSystemMapper = '<Default>'
    SQLDatabaseConfig.DefaultObjectMapper = '<Default>'
    SQLDatabaseConfig.IfTemplate = 'IF <Condition> BEGIN <SQLStatement> END'
    SQLDatabaseConfig.ColumnExistsTemplate = 
      'SELECT * FROM SYS.COLUMNS WHERE UPPER(NAME) = UPPER(N'#39'<ColumnNam' +
      'e>'#39') AND OBJECT_ID = OBJECT_ID(UPPER(N'#39'<TableName>'#39'))'
    SQLDatabaseConfig.TableExistsTemplate = 'SELECT * FROM SYS.TABLES WHERE UPPER(NAME)=UPPER('#39'<TableName>'#39')'
    SQLDatabaseConfig.IndexExistsTemplate = 
      'SELECT NAME FROM SYS.INDEXES WHERE UPPER(NAME)=UPPER('#39'<IndexName' +
      '>'#39') AND OBJECT_ID = OBJECT_ID(UPPER(N'#39'<TableName>'#39'))'
    SQLDatabaseConfig.IndexColumnExistsTemplate = 
      'SELECT IND.NAME FROM SYS.INDEXES IND INNER JOIN SYS.INDEX_COLUMN' +
      'S IC ON  IND.OBJECT_ID = IC.OBJECT_ID AND IND.INDEX_ID = IC.INDE' +
      'X_ID INNER JOIN SYS.COLUMNS COL ON IC.OBJECT_ID = COL.OBJECT_ID ' +
      'AND IC.COLUMN_ID = COL.COLUMN_ID WHERE IND.OBJECT_ID = OBJECT_ID' +
      '(UPPER(N'#39'<TableName>'#39')) AND UPPER(COL.NAME) = UPPER('#39'<IndexColum' +
      'nName>'#39')'
    SQLDatabaseConfig.ColumnTypeForDate = 'DATETIME'
    SQLDatabaseConfig.ColumnTypeForTime = 'DATETIME'
    SQLDatabaseConfig.ColumnTypeForDateTime = 'DATETIME'
    SQLDatabaseConfig.ColumnTypeForBlob = 'VARBINARY(MAX)'
    SQLDatabaseConfig.ColumnTypeForFloat = 'DECIMAL (28,10)'
    SQLDatabaseConfig.ColumnTypeForCurrency = 'DECIMAL (28,10)'
    SQLDatabaseConfig.ColumnTypeForString = 'VARCHAR(%d)'
    SQLDatabaseConfig.ColumnTypeForUnicodeString = 'NVARCHAR(%d)'
    SQLDatabaseConfig.ColumnTypeForText = 'VARCHAR(MAX)'
    SQLDatabaseConfig.ColumnTypeForUnicodeText = 'NVARCHAR(MAX)'
    SQLDatabaseConfig.ColumnTypeForInteger = 'INTEGER'
    SQLDatabaseConfig.ColumnTypeForSmallInt = 'SMALLINT'
    SQLDatabaseConfig.ColumnTypeForInt64 = 'BIGINT'
    SQLDatabaseConfig.ColumnTypeForGUID = 'UNIQUEIDENTIFIER'
    SQLDatabaseConfig.DropColumnTemplate = 
      'DECLARE @CONSTRAINTNAME NVARCHAR(200) SELECT @CONSTRAINTNAME=OD.' +
      'NAME   FROM   SYSOBJECTS OT, SYSCOLUMNS C, SYSOBJECTS OD   WHERE' +
      '  UPPER(OT.NAME) = UPPER('#39'<TableName>'#39')   AND  OT.ID          = ' +
      'C.ID   AND  UPPER(C.NAME)  = UPPER('#39'<ColumnName>'#39')   AND  C.CDEF' +
      'AULT     = OD.ID IF @CONSTRAINTNAME IS NOT NULL   EXEC('#39'ALTER TA' +
      'BLE <TableName> DROP CONSTRAINT '#39' + @CONSTRAINTNAME) IF EXISTS (' +
      'SELECT * FROM SYSCOLUMNS WHERE ID=OBJECT_ID('#39'<TableName>'#39') AND U' +
      'PPER(NAME)=UPPER('#39'<ColumnName>'#39')) EXEC('#39'ALTER TABLE <TableName> ' +
      'DROP COLUMN <ColumnName>'#39')'
    SQLDatabaseConfig.DropTableTemplate = 'DROP TABLE <TableName>'
    SQLDatabaseConfig.IndexInfoTemplate = 
      'SELECT IND.NAME INDEXNAME, IND.IS_PRIMARY_KEY ISPRIMARY, IND.IS_' +
      'UNIQUE ISUNIQUE, COL.NAME COLUMNNAME FROM SYS.INDEXES IND INNER ' +
      'JOIN SYS.INDEX_COLUMNS IC ON IND.OBJECT_ID = IC.OBJECT_ID AND IN' +
      'D.INDEX_ID = IC.INDEX_ID INNER JOIN SYS.COLUMNS COL ON IC.OBJECT' +
      '_ID = COL.OBJECT_ID AND IC.COLUMN_ID = COL.COLUMN_ID WHERE UPPER' +
      '(OBJECT_NAME(IND.OBJECT_ID))=UPPER('#39'<TableName>'#39') ORDER BY INDEX' +
      'NAME, INDEX_COLUMN_ID'
    SQLDatabaseConfig.DropIndexTemplate = 'DROP INDEX <TableName>.<IndexName>'
    SQLDatabaseConfig.EvolveDropsUnknownIndexes = True
    SQLDatabaseConfig.MaxBatchQueryLength = 67108864
    SQLDatabaseConfig.MaxBatchQueryParams = 2000
    SQLDatabaseConfig.BatchQuerySeparator = ';'
    SQLDatabaseConfig.MultiRowInsertLimit = 1000
    SQLDatabaseConfig.SQLforNull = 'NULL'
    SQLDatabaseConfig.SQLforNotNull = 'NOT NULL'
    SQLDatabaseConfig.QuoteNonStringDefaultValues = False
    SQLDatabaseConfig.SupportsConstraintsInCreateTable = True
    SQLDatabaseConfig.SupportsStringDefaultValues = True
    SQLDatabaseConfig.DBGenerationMode = dbgQuery
    SQLDatabaseConfig.DatabaseCaseSensitiveTemplate = 'EXECUTE sp_helpsort'
    SQLDatabaseConfig.ReservedWords.Strings = (
      'ACTIVE, ADD, ALL, AFTER, ALTER'
      'AND, ANY, AS, ASC, ASCENDING,'
      'AT, AUTO, AUTOINC, AVG, BASE_NAME'
      'BEFORE, BEGIN, BETWEEN, BLOB, BOOLEAN,'
      'BOTH, BY, BYTES, CACHE, CAST, CHAR'
      'CHARACTER, CHECK, CHECK_POINT_LENGTH, COLLATE,'
      'COLUMN, COMMIT, COMMITTED, COMPUTED'
      'CONDITIONAL, CONSTRAINT, CONTAINING, COUNT, CREATE, CSTRING,'
      'CURRENT, CURSOR, DATABASE, DATE, DAY'
      'DEBUG, DEC, DECIMAL, DECLARE, DEFAULT,'
      'DELETE, DESC, DESCENDING, DISTINCT, DO'
      'DOMAIN, DOUBLE, DROP, ELSE, END,'
      'ENTRY_POINT, ESCAPE, EXCEPTION, EXECUTE'
      'EXISTS, EXIT, EXTERNAL, EXTRACT, FILE, FILTER,'
      'FLOAT, FOR, FOREIGN, FROM, FULL, FUNCTION'
      'GDSCODE, GENERATOR, GEN_ID, GRANT,'
      'GROUP, GROUP_COMMIT_WAIT_TIME, HAVING'
      'HOUR, IF, IN, INT, INACTIVE, INDEX, INNER,'
      'INPUT_TYPE, INSERT, INTEGER, INTO'
      'IS, ISOLATION, JOIN, KEY, LONG, LENGTH,'
      'LOGFILE, LOWER, LEADING, LEFT, LEVEL'
      'LIKE, LOG_BUFFER_SIZE, MANUAL, MAX, MAXIMUM_SEGMENT,'
      'MERGE, MESSAGE, MIN, MINUTE, MODULE_NAME'
      'MONEY, MONTH, NAMES, NATIONAL, NATURAL,'
      'NCHAR, NO, NOT, NULL, NUM_LOG_BUFFERS'
      'NUMERIC, OF, ON, ONLY, OPTION,'
      'OR, ORDER, OUTER, OUTPUT_TYPE, OVERFLOW'
      'PAGE_SIZE, PAGE, PAGES, PARAMETER, PASSWORD,'
      'PLAN, POSITION, POST_EVENT, PRECISION'
      
        'PROCEDURE, PROTECTED, PRIMARY, PRIVILEGES, RAW_PARTITIONS, RDB$D' +
        'B_KEY,'
      'READ, REAL, RECORD_VERSION, REFERENCES'
      'RESERV, RESERVING, RETAIN, RETURNING_VALUES, RETURNS, REVOKE,'
      'RIGHT, ROLE, ROLLBACK, SECOND, SEGMENT'
      'SELECT, SET, SHARED, SHADOW, SCHEMA, SINGULAR,'
      'SIZE, SMALLINT, SNAPSHOT, SOME, SORT'
      'SQLCODE, STABILITY, STARTING, STARTS, STATISTICS,'
      'SUB_TYPE, SUBSTRING, SUM, SUSPEND, TABLE'
      'THEN, TIME, TIMESTAMP, TIMEZONE_HOUR, TIMEZONE_MINUTE,'
      'TO, TRAILING, TRANSACTION, TRIGGER, TRIM'
      'UNCOMMITTED, UNION, UNIQUE, UPDATE, UPPER,'
      'USER, VALUE, VALUES, VARCHAR, VARIABLE'
      'VARYING, VIEW, WAIT, WHEN, WHERE,'
      'WHILE, WITH, WORK, WRITE, YEAR')
    SQLDatabaseConfig.StoreEmptyStringsAsNULL = False
    SQLDatabaseConfig.SystemTablePrefix = 'BOLD'
    SQLDatabaseConfig.QuoteLeftBracketInLike = False
    SQLDatabaseConfig.SqlScriptTerminator = ';'
    SQLDatabaseConfig.SqlScriptCommentStart = '/* '
    SQLDatabaseConfig.SqlScriptCommentStop = ' */'
    SQLDatabaseConfig.SqlScriptStartTransaction = 'START TRANSACTION'
    SQLDatabaseConfig.SqlScriptCommitTransaction = 'COMMIT'
    SQLDatabaseConfig.SqlScriptRollBackTransaction = 'ROLLBACK'
    CustomIndexes = <>
    Connection = UniConnection1
    DatabaseEngine = dbeSQLServer
    Left = 616
    Top = 192
  end
  object UniConnection1: TUniConnection
    ProviderName = 'SQL Server'
    Database = 'bldown22'
    Username = 'dba'
    Server = 'localhost'
    Left = 616
    Top = 240
    EncryptedPassword = '92FF9EFF8CFF8BFF9AFF8DFF94FF9AFF86FF'
  end
end
