...
unit TestModel1_Temporal;

{$DEFINE TestModel1_Temporal_unitheader}
{$INCLUDE TestModel1_Temporal_Interface.inc}
...
function TBusinessClassesRoot.AtTime(Time: TBoldTimestampType): TBusinessClassesRoot;
begin
  result := TBusinessClassesRoot(inherited AtTime(Time));
end;
...
function TClassA._Get_M_aBlobContent: TBAString;
begin
  assert(ValidateMember('TClassA', 'aBlobContent', 13, TBAString));
...
function TClassA.AtTime(Time: TBoldTimestampType): TClassA;
begin
  result := TClassA(inherited AtTime(Time));
end;
...
function THitList.AtTime(Time: TBoldTimestampType): THitList;
begin
  result := THitList(inherited AtTime(Time));
end;
...
function TLinkClass.AtTime(Time: TBoldTimestampType): TLinkClass;
begin
  result := TLinkClass(inherited AtTime(Time));
end;
...
function TSong.AtTime(Time: TBoldTimestampType): TSong;
begin
  result := TSong(inherited AtTime(Time));
end;
...
begin
  assert(ValidateMember('TThing', 'one', 0, TBoldObjectReference));
  Result := TBoldObjectReference(BoldMembers[0]);
end;
...
begin
  assert(ValidateMember('TThing', 'oneLinkClass', 1, TBoldObjectReference));
  Result := TBoldObjectReference(BoldMembers[1]);
end;
...
begin
  assert(ValidateMember('TThing', 'many', 2, TThingList));
  Result := TThingList(BoldMembers[2]);
end;

function TThing._GetmanyLinkClass: TLinkClassList;
begin
  assert(ValidateMember('TThing', 'manyLinkClass', 3, TLinkClassList));
  Result := TLinkClassList(BoldMembers[3]);
end;

function TThing.AtTime(Time: TBoldTimestampType): TThing;
begin
  result := TThing(inherited AtTime(Time));
end;
...
begin
  assert(ValidateMember('ThitListsong', 'song', 0, TBoldObjectReference));
  Result := TBoldObjectReference(BoldMembers[0]);
end;
...
begin
  assert(ValidateMember('ThitListsong', 'hitList', 1, TBoldObjectReference));
  Result := TBoldObjectReference(BoldMembers[1]);
end;
...
function ThitListsong.AtTime(Time: TBoldTimestampType): ThitListsong;
begin
  result := ThitListsong(inherited AtTime(Time));
end;
...
function partpartof.AtTime(Time: TBoldTimestampType): partpartof;
begin
  result := partpartof(inherited AtTime(Time));
end;
...
initialization
  CodeDescriptor := GeneratedCodes.AddGeneratedCodeDescriptorWithFunc('TestModel1_Temporal', InstallBusinessClasses, InstallObjectListClasses, GeneratedCodeCRC);
finalization
